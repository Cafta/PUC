...
*******************************************************************************/
# 28 "C:\\GitHub\\PUC\\Seguidor_de_linha\\arduino\\arduino.ino"
# 29 "C:\\GitHub\\PUC\\Seguidor_de_linha\\arduino\\arduino.ino" 2
...
void loop() {
  ...
  if((leftValue > 700) && (rightValue > 700)) {
   leftSpeed = 50 /* Set to any number from 0 - 100.*/;
   rightSpeed = 50 /* Set to any number from 0 - 100.*/;
  }

  // if the line only is under the right sensor, adjust relative speeds to turn to the right
  else if(rightValue > 700) {
   leftSpeed = 50 /* Set to any number from 0 - 100.*/ + 40;
   rightSpeed = 50 /* Set to any number from 0 - 100.*/ - 40;
  }

  // if the line only is under the left sensor, adjust relative speeds to turn to the left
  else if(leftValue > 700) {
   leftSpeed = 50 /* Set to any number from 0 - 100.*/ - 40;
   rightSpeed = 50 /* Set to any number from 0 - 100.*/ + 40;
  }

  // run motors given the control speeds above
  motors.leftDrive(leftSpeed, 1);
  motors.rightDrive(rightSpeed, 1);
